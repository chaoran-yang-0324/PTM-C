...
def run_IsometricWork(folder_path, start_cutoff=40, end_cutoff=200, baseline_cutoff=45):

    def IsometricWork(file_path):
        ...
        y = data_array[:,2][start_cutoff:end_cutoff]*99.06 # force, currently in mN

        baseline_start = y[0:baseline_cutoff] # edit this based on "int_sg"

        mean_start = np.mean(baseline_start)
        sd_start = np.std(baseline_start)

        index_start = None
        for i in range(160):
            if y[i] > mean_start+5*sd_start:
                index_start = i-3
                break

        baseline_end = y[end_cutoff-start_cutoff-baseline_cutoff:end_cutoff-start_cutoff] # edit this based on "int_sg"

        mean_end = np.mean(baseline_end)
        x_adj = np.copy(x)
        y_adj = np.copy(y)
        ...
        for i in range(160):
            if y[i] > mean_end:
                ...
                with warnings.catch_warnings():
                    ...
                    popt, pcov = curve_fit(linear_fit_func, x[index_end-1:index_end+1], y[index_end-1:index_end+1])
                x_adj[i]=(mean_end - popt[1])/popt[0]
                break

        integral = np.trapezoid(y_adj[index_start:index_end+1]-mean_start, x_adj[index_start:index_end+1])
            ...
            if os.path.isdir(mouse_files):
                for f in os.listdir(mouse_files):
                    if f.lower().endswith(".xlsx") or f.lower().endswith(".xls"):
                        ...
                        d=excel_path.iloc[9,1] # CSA (cm^2)
    ...
    ax.set_ylabel('Work Normalized by CSA (cm^2)')
    ax.set_title('Total Isometric Work')
    ax.legend()
    ax.grid()
...
start_cutoff = st.number_input("Start Cutoff:", min_value=0, value=40, step=1)
end_cutoff = st.number_input("End Cutoff:", min_value=start_cutoff+1, value=200, step=1)
...
if st.button("Run Analysis"):
    ...
    fig,csv_output = run_IsometricWork(unzip_folder,start_cutoff,end_cutoff,baseline_cutoff)
    st.write("Graphing...")
    st.pyplot(fig)
